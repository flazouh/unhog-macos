...
public struct CulpritPreferences: Codable, Equatable, Sendable {
    ...
    public static let recommended = CulpritPreferences()
}
...
public struct PreferencePolicies: Equatable, Sendable {
    ...
    public static func make(
        from preferences: CulpritPreferences,
...
public final class UserDefaultsPreferencesRepository {
    ...
    public init(
        ...
        storageKey: String = "culprit.preferences.v1"
    ) {
        self.defaults = defaults
        self.storageKey = storageKey
    }

    public func load() -> CulpritPreferences {
        if let data = defaults.data(forKey: storageKey),
           let preferences = try? JSONDecoder().decode(
               CulpritPreferences.self,
               from: data
           ) {
            return preferences
        }

        var preferences = CulpritPreferences.recommended
    ...
    public func save(_ preferences: CulpritPreferences) {
